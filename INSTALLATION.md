...
**Version:** 0.1.0  
**Installer:** `Filip.DHCP-0.1.0-x64.msi`  
...
```cmd
msiexec /i Filip.DHCP-0.1.0-x64.msi
```
...
```cmd
msiexec /i Filip.DHCP-0.1.0-x64.msi /qn /norestart
```

Silent installation with detailed log:

```cmd
msiexec /i Filip.DHCP-0.1.0-x64.msi /qn /norestart /L*v C:\Windows\Temp\Filip.DHCP-install.log
```

Recommended for troubleshooting:

```cmd
msiexec /i Filip.DHCP-0.1.0-x64.msi /passive /norestart /L*v C:\Windows\Temp\Filip.DHCP-install.log
```
...
```cmd
msiexec /x Filip.DHCP-0.1.0-x64.msi /qn /norestart
```

Uninstall with log:

```cmd
msiexec /x Filip.DHCP-0.1.0-x64.msi /qn /norestart /L*v C:\Windows\Temp\Filip.DHCP-uninstall.log
```
...
```text
\\server\software\Filip.DHCP\Filip.DHCP-0.1.0-x64.msi
```

Silent install command:

```cmd
msiexec /i "\\server\software\Filip.DHCP\Filip.DHCP-0.1.0-x64.msi" /qn /norestart /L*v C:\Windows\Temp\Filip.DHCP-install.log
```
...
```powershell
Get-AuthenticodeSignature ".\Filip.DHCP-0.1.0-x64.msi"
```
